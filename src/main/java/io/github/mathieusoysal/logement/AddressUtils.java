...
class AddressUtils {
    ...
    private static final Pattern PATTER_REGEX_ZIP_CODE = Pattern.compile("\\d+(?=[a-zA-Z]*[^\\d]*[\\d ]*$)",
            Pattern.MULTILINE);
    private static final Pattern PATTERN_REGEX_STREET = Pattern.compile("^.*[^ \\-](?= +[a-zA-Z]*[- ]*[\\d]{4,} +[^\\d]*[\\d ]*$)",
            Pattern.MULTILINE);
